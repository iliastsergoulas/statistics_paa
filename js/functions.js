...
function mylines(t) {
    if ("el-GR" == t) {
        ...
        e = 1100 - (d = {
            ...
            bottom: 70,
            left: 80
        }).left - d.right, a = 400 - d.top - d.bottom, n = d3.time.format("%Y-%m-%d").parse, r = d3.time.scale().range([0, e]), l = d3.scale.linear().range([a, 0]), o = d3.svg.axis().scale(r).ticks(d3.time.months, 3).tickFormat(function(t) {
            var e = t.getTime() - 1e4,
                a = new Date(e),
                n = a.getMonth(),
                r = a.getFullYear();
	...
	d3.json('http://data.agrotikianaptixi.gr/myindex.php/eu', function(error, mydata) {
		...
		if (language=='el-GR'){
			...
			svg_eu.append("text")
				...
				.style("fill", "#35a745")  
				.text("Ποσοστό απορρόφησης ΠΑΑ στην Ε.Ε.");
			svg_eu.append("text")
				.attr("x", (widtheu / 8))             
				.attr("y", y_eu(0.3368)-10)
				.attr("text-anchor", "middle")  
				.style("font-size", "95%")
				.style("fill", "red")
				.text("Μέσος όρος Ε.Ε.");
			svg_eu.append("text")
				...
				.text("Πηγή: Rural Development Committee 09/2018");
				var tip = d3.tip()
					.attr('class', 'd3-tip')
					.offset([-10, 0])
					.html(function(d) {
			...
			svg_eu.append("text")
				...
				.style("font-weight", "bold")
				.style("fill", "#35a745")  
				.text("RDP financial execution in EU");
			svg_eu.append("text")
				.attr("x", (widtheu / 8))             
				.attr("y", y_eu(0.3368)-10)
				.attr("text-anchor", "middle")  
			...
			svg_eu.append("text")
				...
				.style("font-size", "50%") 
				.style("text-decoration", "underline")  
				.text("Source: Rural Development Committee 09/2018");
			var tip = d3.tip()
				.attr('class', 'd3-tip')
				.offset([-10, 0])
				.html(function(d) {
	...
	d3.json('http://data.agrotikianaptixi.gr/myindex.php/eu', function(error, mydata) {
		...
		if (language=='el-GR'){
			...
			svg_eupayments.append("text")
				...
				.style("font-weight", "bold")
				.style("fill", "#35a745")  
				.text("Πληρωμές ΠΑΑ στην Ε.Ε. (δις €)");
			svg_eupayments.append("text")
				.attr("x", (widtheupayments / 8))             
				.attr("y", y_eupayments(1.2)-10)
				.attr("text-anchor", "middle")  
				.style("font-size", "95%")
				.style("fill", "red")
				.text("Μέσος όρος Ε.Ε.");
		...
		svg_eupayments.append("line")
			...
			.style("stroke-width", "2")
			.attr("x1", 0)
			.attr("y1", y_eupayments(1.2))
			.attr("x2", widtheupayments)
			.attr("y2", y_eupayments(1.2));
		});
};
